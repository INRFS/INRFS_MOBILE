...
//     // dropped on the floor with a console.log.
//     registerInvestor({
//       name: form.fullName,
//       mobile: form.mobile,
//       email: form.email,
...
const RegistrationScreen = ({navigation}: any) => {
  ...
  // FIX: registerInvestor() already generates the real Investor ID at
  // registration time and returns it — this screen just used to throw
  // that value away. Without a backend/email yet, there was no way for
  // the person to know their real ID, so they'd type a made-up one at
  // Login later, which never matched the actual registered record (that's
  // what was causing the generic "Investor" name / blank branch to show
  // up on the admin's Investment Management screen). Capturing and
  // displaying it here closes that gap — it's demo-only surfacing of
  // something the backend will later deliver by email.
      ...
      <Modal
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            {/* FIX: surface the real generated Investor ID for demo use.
                In production this arrives by email after admin approval —
                shown here only because there's no backend/email yet, so
                without this the person has no way to log in with the ID
                that actually matches their registered record. */}
            {registeredId && (
              <View style={styles.checklistCard}>
                <Text style={styles.checklistHeading}>Your Investor ID (demo)</Text>
                <Text
                  style={[
                    styles.checklistTitle,
                    {fontSize: 18, marginTop: 4, marginBottom: 4},
                  ]}>
                  {registeredId}
                </Text>
                <Text style={styles.checklistDesc}>
                  Save this. Once approved, log in with this ID and the mobile number you
                  registered with.
                </Text>
              </View>
            )}
