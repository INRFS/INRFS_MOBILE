...
// `investors` array in context instead of the old STATIC_INVESTORS seed,
// the same way PaymentQueueScreen derives its rows from `payouts`.
interface InvestorRow {
  id: string;
  name: string;
  mobile: string;
...
// Investor (from AppNavigator context) has no "registered on" date field —
// rather than touching the shared Investor type (which every other screen
// also reads), we fall back to '—', mirroring how PaymentQueueScreen
// already falls back to '—' for its own missing `reference` field.
const toInvestorRow = (inv: Investor): InvestorRow => ({
  id: inv.id,
  name: inv.name,
  mobile: inv.mobile && inv.mobile !== '—' ? inv.mobile : '—',
  branch: inv.branch && inv.branch !== '—' ? inv.branch : '—',
  registered: '—',
  kyc: inv.kycStatus,
  status: inv.status,
  aum: formatINR(inv.totalInvested),
});
...
const InvestorManagementScreen = ({navigation}: any) => {
  ...
  // Live investors from context (populated at registration via
  // registerInvestor, same source AdminDashboard's Investor Registry reads
  // from) mapped into the row shape this screen already renders.
  const investorRows = investors.map(toInvestorRow);

  const rows = investorRows.filter(
    inv =>
      inv.name.toLowerCase().includes(query.toLowerCase()) ||
      inv.id.toLowerCase().includes(query.toLowerCase()),
  );
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        <View style={styles.headerRow}>
          <View>
            <Text style={styles.title}>Investor Management</Text>
            <Text style={styles.subtitle}>All investors across all branches — {investorRows.length} records</Text>
          </View>
        </View>
